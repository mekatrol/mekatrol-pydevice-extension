...
const excludedLabel = msg('excluded', 'Excluded');
const excludedNote = msg('excludedPathNote', 'this path is configured to be excluded by default');
const excludedIconSvg = '<svg viewBox="0 0 16 16" aria-hidden="true"><path d="M2.2 2.2a.75.75 0 0 1 1.06 0l10.54 10.54a.75.75 0 1 1-1.06 1.06L2.2 3.26a.75.75 0 0 1 0-1.06Zm5.8.8c3.2 0 5.86 1.73 7.36 4.52a1 1 0 0 1 0 .96 8.43 8.43 0 0 1-2.2 2.71l-1.08-1.08c.78-.55 1.45-1.3 1.95-2.11-1.27-2.07-3.4-3.5-6.03-3.5-.94 0-1.82.19-2.63.53l-1.2-1.2A8.5 8.5 0 0 1 8 3Zm-6.36 4.52A8.43 8.43 0 0 1 4 4.8l1.1 1.1c-.78.54-1.45 1.28-1.96 2.1 1.27 2.08 3.4 3.5 6.03 3.5.95 0 1.84-.19 2.66-.54l1.2 1.2A8.48 8.48 0 0 1 8 13a8.36 8.36 0 0 1-7.36-4.52 1 1 0 0 1 0-.96ZM8 6a2 2 0 0 1 2 2c0 .24-.04.47-.12.68l-2.56-2.56A1.97 1.97 0 0 1 8 6Zm-2 2c0-.24.04-.47.12-.68l2.56 2.56A2 2 0 0 1 6 8Z"></path></svg>';
...
const isChecked = (row) => {
  ...
  if (checkedById.has(row.id)) {
    return !!checkedById.get(row.id);
  }
  return !row.excluded;
};
  ...
  for (const row of nextRows) {
    ...
    checkbox.addEventListener('change', () => {
      ...
      if (checkbox.dataset.dir === 'true' && checkbox.checked === false) {
        const prefix = `${row.deviceRelativePath}/`;
        for (const item of rows) {
          if (item.deviceRelativePath.startsWith(prefix)) {
            checkedById.set(item.id, false);
          }
        }
      }
      renderRows(rows);
    ...
    stateTd.className = 'state status-' + (row.excluded ? 'excluded' : row.status);
    const stateWrap = document.createElement('span');
    stateWrap.className = 'status-wrap';
    const stateIcon = document.createElement('span');
    stateIcon.className = 'icon';
    stateIcon.innerHTML = row.excluded ? excludedIconSvg : labels[row.status].iconSvg;
    const stateText = document.createElement('span');
    stateText.textContent = row.excluded ? excludedLabel : labels[row.status].text;
    stateWrap.appendChild(stateIcon);
    stateWrap.appendChild(stateText);
    stateTd.appendChild(stateWrap);
    tr.appendChild(stateTd);

    const runTd = document.createElement('td');
    runTd.className = 'status';
    if (row.runStatus) {
      runTd.classList.add('run-' + row.runStatus);
      runTd.textContent = runLabels[row.runStatus] || row.runStatus;
      if (row.runErrorText) {
        runTd.title = row.runErrorText;
      }
    }
