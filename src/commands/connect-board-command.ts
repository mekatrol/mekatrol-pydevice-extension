...
const saveDirtyDeviceDocumentsBeforeDisconnect = async (deviceId?: string): Promise<boolean> => {
  ...
  if (action !== 'Save & Disconnect') {
    outputChannelLogger.log('Disconnect cancelled by user: unsaved device files were not saved.', true);
    return false;
  }
...
const closeOpenDeviceTabsAfterDisconnect = async (deviceId?: string): Promise<void> => {
  ...
  if (!closed) {
    outputChannelLogger.log('Disconnected, but some device tabs could not be closed.', true);
  }
};
...
const pruneStaleConnectedDevices = async (
  ...
  if (!activePorts) {
    try {
      ...
      const reason = error instanceof Error ? error.message : String(error);
      outputChannelLogger.log(`Skipping stale connection pruning: unable to list serial devices. ${reason}`, false);
      return;
    }
  }
    ...
    const applyRefreshedRuntimeInfo = async (refreshedRuntimeInfo: PyDeviceRuntimeInfo): Promise<void> => {
      ...
      if (promotedDeviceId !== previousDeviceId) {
        if (boardRegistry.hasDeviceId(promotedDeviceId)) {
          ...
          void (async () => {
            try {
              await removeDeviceMirror(previousDeviceId);
              await syncDeviceToMirror(state.board, promotedDeviceId);
              outputChannelLogger.log(`Device mirror synced for promoted device ID ${promotedDeviceId}.`, false);
            } catch (error: unknown) {
              const reason = error instanceof Error ? error.message : String(error);
...
export const closeConnectedPyDeviceByDeviceId = async (
  ...
  try {
    ...
    if (showSuccessMessage) {
      ...
      outputChannelLogger.log(`Board connection closed for ${deviceId} during extension shutdown.`, false);
    }
  } catch (error) {
    const reason = error instanceof Error ? error.message : String(error);
    ...
    if (alreadyClosed) {
      ...
      outputChannelLogger.log(`Board connection for ${deviceId} was already closed. Cleared stale state.`, true);
      return true;
    }
      ...
      try {
        for (let index = 0; index < availablePorts.length; index += 1) {
          ...
          try {
            ...
            const reason = error instanceof Error ? error.message : String(error);
            outputChannelLogger.log(`Probe failed for ${port.path}: ${reason}`, false);
            probedRuntimeInfoByPath.delete(port.path);
            emitPyDeviceLoggerEvent({
              source: probeLoggerSource,
              level: 'debug',
...
export const tryReconnectBoardOnStartup = async (context: vscode.ExtensionContext): Promise<void> => {
  ...
  try {
    ...
    const reason = error instanceof Error ? error.message : String(error);
    outputChannelLogger.log(`Auto reconnect could not list serial ports before reconnecting: ${reason}`, false);
  }
  ...
  try {
    for (const devicePath of startupReconnectPaths) {
      ...
      try {
        ...
        outputChannelLogger.log(`Auto reconnect skipped for ${devicePath}: ${reason}`, false);
      }
    }
  } finally {
    if (!hadFailures) {
